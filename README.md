...
# svm
mod <- wrap_svm(as.matrix(X_multi_train), y_multi_train, kernel = "radial")
pred_multi_svm <- predict(mod, newx = as.matrix(X_multi_test), type = "class")
...
# xgboost
mod <- wrap_xgboost(as.matrix(X_reg_train), y_reg_train,
                    nrounds = 50, objective = "reg:squarederror", verbose = 0)
pred_reg_xgboost <- predict(mod, newx = as.matrix(X_reg_test))
rmse_xgboost <- sqrt(mean((pred_reg_xgboost - y_reg_test)^2))

# lightgbm
mod <- wrap_lightgbm(as.matrix(X_reg_train), y_reg_train,
                     params = list(objective = "regression", verbose = -1),
                     nrounds = 50)
pred_reg_lightgbm <- predict(mod, newx = as.matrix(X_reg_test))
rmse_lightgbm <- sqrt(mean((pred_reg_lightgbm - y_reg_test)^2))

# glmnet
mod <- wrap_glmnet(as.matrix(X_reg_train), y_reg_train, alpha = 0)
pred_reg_glmnet <- predict(mod, newx = as.matrix(X_reg_test))
rmse_glmnet <- sqrt(mean((pred_reg_glmnet - y_reg_test)^2))

# svm
mod <- wrap_svm(as.matrix(X_reg_train), y_reg_train)
pred_reg_svm <- predict(mod, newx = as.matrix(X_reg_test))
